...
export default function NewPaymentPage() {
  ...
  useEffect(() => {
    async function fetchLink() {
      ...
      setError(null);
      try {
        // For public payment links, we need a public endpoint
        // For now, we'll use the existing endpoint but this should be made public
  ...
  const handleConnectWallet = async () => {
    try {
      ...
      // Only show success toast if we actually connected a wallet
  ...
  const handlePay = async () => {
    ...
    try {
      if (paymentMethod === 'onchain') {
        ...
        // Use the public payment endpoint that doesn't require authentication
        const receiptData = await paymentLinkService.processPayment({
          linkId: linkId,
          customFieldData: {
            amount,
            customFields: customFieldValues,
